# Go Cloud Functions on EdgeOne Pages - Fiber 框架

一个基于 Next.js + Tailwind CSS 前端和 Go Fiber 后端的全栈演示应用，展示如何在 EdgeOne Pages 上使用 Fiber 框架部署 Go 云函数，支持 RESTful API 路由。

## 🚀 特性

- **Fiber 框架集成**：基于 fasthttp 的 Express 风格 Go Web 框架，支持中间件、JSON 解析和路由组
...
### 后端
- **Go 1.21** - 云函数运行时
- **Fiber v2** - 基于 fasthttp 的 Express 风格 Go Web 框架
...
│   ├── api.go                     # Fiber 应用，包含所有 REST API 路由
│   ├── go.mod                     # Go 模块定义
│   └── go.sum                     # Go 依赖校验和
├── src/
...
所有 API 端点定义在单个 `cloud-functions/api.go` 文件中，使用 Fiber 的路由组：
...
Go 后端使用 Fiber 的标准模式 — 路由组、Body 解析和内置中间件：
...
- **Fiber 框架文档**：[https://docs.gofiber.io](https://docs.gofiber.io)
...
在项目根目录创建 `cloud-functions/api.go`，编写 Fiber 应用：
...
[![Deploy with EdgeOne Pages](https://cdnstatic.tencentcs.com/edgeone/pages/deploy.svg)](https://console.cloud.tencent.com/edgeone/pages/new?from=github&template=go-fiber)
